...
-- Force replace function to ensure meeting_location is respected
...
BEGIN
  ...
  -- Logic: Calendly creates 'pending' booking first. Stripe confirmation should hijack it.
  ...
  IF v_existing_id IS NOT NULL THEN
     ...
     SET 
        status = p_status,
        source = p_source, -- 'stripe'
        amount = p_amount,
        variant_id = COALESCE(variant_id, p_variant_id), 
        external_id = p_external_id,
        booking_date = p_booking_date, -- Payment date
        -- CRITICAL: Update meeting_location if provided (from Stripe metadata), otherwise keep what Calendly set
        meeting_location = COALESCE(p_meeting_location, meeting_location),
        updated_at = now()
     WHERE id = v_existing_id
     RETURNING id INTO v_booking_id;
  ELSE
     -- INSERT New Booking (Standard Flow without pre-scheduling)

...
CREATE TYPE offer_status AS ENUM ('draft', 'pending', 'approved', 'rejected', 'active');
...
-- Table des QR codes utilisateurs
...
-- Politiques RLS pour club_events
...
CREATE POLICY "Users can manage their registrations"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM user_profiles 
      WHERE user_profiles.user_id = auth.uid() 
      AND user_profiles.id = event_registrations.user_id
    )
  )
  WITH CHECK (
    EXISTS (
...
-- Politiques RLS pour masterclasses
CREATE POLICY "Premium members can access masterclasses"
  ...
  USING (
    EXISTS (
      ...
      WHERE user_profiles.user_id = auth.uid() 
      AND user_profiles.subscription_status = 'active'
      AND user_profiles.subscription_type = 'premium'
    )
  );

-- Politiques RLS pour masterclass_attendees
CREATE POLICY "Users can manage their masterclass attendance"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM user_profiles 
      WHERE user_profiles.user_id = auth.uid() 
      AND user_profiles.id = masterclass_attendees.user_id
    )
  )
  WITH CHECK (
    EXISTS (
...
-- Politiques RLS pour wellness_consultations
CREATE POLICY "Users can manage their consultations"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM user_profiles 
      WHERE user_profiles.user_id = auth.uid() 
      AND user_profiles.id = wellness_consultations.user_id
    )
  )
  WITH CHECK (
    EXISTS (
...
-- Politiques RLS pour club_boxes
...
-- Politiques RLS pour box_shipments
CREATE POLICY "Users can view their shipments"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM user_profiles 
      WHERE user_profiles.user_id = auth.uid() 
      AND user_profiles.id = box_shipments.user_id
    )
  );

-- Politiques RLS pour member_rewards
CREATE POLICY "Users can view their rewards"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM user_profiles 
      WHERE user_profiles.user_id = auth.uid() 
      AND user_profiles.id = member_rewards.user_id
    )
  );

-- Politiques RLS pour community_challenges
...
-- Politiques RLS pour challenge_participations
CREATE POLICY "Users can manage their challenge participation"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM user_profiles 
      WHERE user_profiles.user_id = auth.uid() 
      AND user_profiles.id = challenge_participations.user_id
    )
  )
  WITH CHECK (
    EXISTS (
...
-- Politiques RLS pour user_qr_codes
CREATE POLICY "Users can view own QR code"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM user_profiles 
      WHERE user_profiles.user_id = auth.uid() 
      AND user_profiles.id = user_qr_codes.user_profile_id
    )
  );
...
-- Fonction pour vérifier si un utilisateur est membre premium
...
-- Fonction pour mettre à jour les participants aux événements
...
-- Fonction pour mettre à jour les participants aux masterclasses
...
-- Fonction pour mettre à jour les participants aux défis
...
-- Fonction pour créer automatiquement les récompenses membre

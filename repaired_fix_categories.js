...
async function ensureCategory(slug, name, parentSlug = null, parentName = null) {
    // Check if exists
    const { data } = await supabase.from('offer_categories').select('*').eq('slug', slug).limit(1);
    ...
    const { data: newData, error } = await supabase.from('offer_categories').insert(payload).select().single();
...
async function syncCategories() {
    console.log('Syncing categories safely...');

    for (const cat of categories) {
        await ensureCategory(cat.slug, cat.name, null, null);

        if (cat.subcategories) {
            for (const sub of cat.subcategories) {
                await ensureCategory(sub.slug, sub.name, cat.slug, cat.name);
            }
        }
    }

...
async function createPartner(partner: typeof partners[0]) {
    ...
    if (!partnerId) {
        ...
        const { data: newPartner, error: partnerError } = await supabase
            ...
            .insert({
                ...
                category: partner.category, // Assuming category column exists or is handled loosely
                status: 'approved',
    ...
    const { error: profileError } = await supabase
        ...
        .upsert({
            ...
            role: 'partner' // Ensure role is set if applicable, though usually derived
        }, { onConflict: 'user_id' });

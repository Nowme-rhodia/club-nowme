// check-supabase-config.js - Version StackBlitz (corrigé)
...
const serviceRoleKey = process.env.SUPABASE_SERVICE_ROLE_KEY || window.env?.SUPABASE_SERVICE_ROLE_KEY;
...
async function checkSupabaseConfig() {
  ...
  requiredEnvVars.forEach(envVar => {
    const value = process.env[envVar] || window.env?.[envVar];
...
function createUI() {
  ...
  description.textContent = 'Cet outil vérifie la configuration de Supabase dans votre environnement StackBlitz.';

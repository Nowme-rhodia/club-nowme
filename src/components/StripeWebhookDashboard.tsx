...
import { Badge, Button, Card } from './ui'; // Supposons que vous avez des composants UI
                ...
                events.map((event) => (
                  <tr key={event.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {event.customer_email || event.customer_id || '-'}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
        ...
        <div className="flex space-x-2">
          <button
            ...
            className={`px-3 py-1 border rounded ${
              canGoPrevious ? 'bg-white hover:bg-gray-50' : 'bg-gray-100 cursor-not-allowed'
            }`}
          >
            Précédent
          </button>
          <button
            ...
            disabled={!canGoNext}
            className={`px-3 py-1 border rounded ${
              canGoNext ? 'bg-white hover:bg-gray-50' : 'bg-gray-100 cursor-not-allowed'
            }`}
          >
            Suivant
          </button>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-3xl w-full max-h-[80vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div>
                <p className="text-sm font-medium text-gray-500">ID Événement</p>
                <p className="text-sm">{selectedEvent.stripe_event_id}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-sm">{selectedEvent.customer_id || '-'}</p>
              </div>
              <div>
                <p className="text-sm font-medium text-gray-500">ID Abonnement</p>
                <p className="text-sm">{selectedEvent.subscription_id || '-'}</p>
              </div>
              <div>
                <p className="text-sm font-medium text-gray-500">Montant</p>
                <p className="text-sm">{formatAmount(selectedEvent.amount)}</p>
              </div>
              <div>
                <p className="text-sm font-medium text-gray-500">Statut</p>
                <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusColor(selectedEvent.status)}`}>

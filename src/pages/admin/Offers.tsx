...
interface Offer {
  ...
  location: string;
  status: 'draft' | 'pending' | 'approved' | 'rejected' | 'active' | 'ready';
  is_approved?: boolean;
  created_at: string;
  updated_at: string;
  partner: {
  ...
  const filteredOffers = offers
    .filter(offer => {
      ...
      const matchesStatus = statusFilter === 'all' ||
        (statusFilter === 'active' && offer.is_approved !== false && offer.status !== 'draft' && offer.status !== 'ready' && offer.status !== 'pending' && offer.status !== 'rejected') ||
        (statusFilter === 'inactive' && offer.is_approved === false && offer.status !== 'draft' && offer.status !== 'ready' && offer.status !== 'pending') ||
        (statusFilter === 'draft' && offer.status === 'draft') ||
        (statusFilter === 'ready' && offer.status === 'ready') ||
        ...
        <ul className="divide-y divide-gray-200">
          {filteredOffers.map((offer) => {
            ...
            if (offer.status === 'draft') {
              ...
              // Approved or Active
              statusBadge = (
                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${isActive ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-500'
                  }`}>
